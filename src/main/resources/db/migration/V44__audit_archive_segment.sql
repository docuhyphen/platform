-- Phase 4 (Immutable WORM Archive + Signed Segments + Verification) of
...
(
    ...
    -- production, a local dev keypair otherwise) - never KMS/HSM in this phase, see the Phase 4
    ...
    -- True only for segments produced by the one-time legacy-row importer (task 5): these carry
...
-- Phase 4 note: audit_ledger_event.signing_key_id / checkpoint_ref (added nullable in
-- V42__audit_ledger.sql, "populated starting Phase 4") are deliberately left NULL by this phase
...
-- not an oversight; flagged again in AUDIT-ARCHITECTURE-IMPLEMENTATION.md's Phase 4 handoff.
