 ...
 * columns on `sharing_session` (recipient_id / recipient_type / group_id) and the per-session
...
)
{
     ...
     * pure participant. Replaces the legacy single `SharingSession.recipient`.
     */
    fun recipientShares(sessionId: UUID): List<Share> =
        shareRepository.findActiveByResource(ResourceType.SHARING_SESSION, sessionId)
            .filter {
     ...
     */
    private fun allRecipientShares(sessionId: UUID): List<Share> =
        shareRepository.findActiveByResource(ResourceType.SHARING_SESSION, sessionId)
            .filter {
                it.roleName != RoleName.OWNER.name
            }

    fun recipientUserIds(sessionId: UUID): List<UUID> =
        recipientShares(sessionId).map { it.principalId }.distinct()

    fun primaryRecipientUserId(sessionId: UUID): UUID? = recipientShares(sessionId).firstOrNull()?.principalId

    /** Whether the session's recipient share permits the given constraint flag (e.g. "allow_document_upload"). */
    fun recipientConstraintAllows(sessionId: UUID, flag: String): Boolean
    {
        val constraints = allRecipientShares(sessionId).firstOrNull()?.constraintsJson ?: return false
        return constraints.contains("\"$flag\":true")
    }

    /** Returns the constraints JSON of the primary recipient share, or null if none. */
    fun recipientConstraintsJson(sessionId: UUID): String? =
        allRecipientShares(sessionId).firstOrNull()?.constraintsJson
     ...
     */
    fun updateRecipientConstraints(sessionId: UUID, constraintsJson: String)
    {
        val shares = allRecipientShares(sessionId)
        if (shares.isEmpty())
        {
            logger.warn("updateRecipientConstraints: no recipient shares found for session={}", sessionId)
            return
        }
        for (share in shares)
        {
