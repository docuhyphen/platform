...
)
{
    ...
    ): OrganizationExchangeLink
    {
        ...
        val createdLink = organizationExchangeLinkRepository.save(link)
    ...
    ): OrganizationExchangeLink
    {
        ...
        val acceptedLink = organizationExchangeLinkRepository.update(link)
            ...
            link.requestingOrganization?.id?.let { orgId ->
                ...
                if (userRoleService.isOrgAdminIn(orgAppUser.id, orgId))
                {
                    emailService.sendEmail(
                        ...
                        """
                            Your paring request to ${link.requestedOrganization?.name} has been accepted. You may now start exchanging documents.
                            """.trimIndent()
                    )
                }
            } }
    ...
    fun getLinksByOrganization(organizationId: String?): List<OrganizationExchangeLink>?
    {
        ...
        return with(organizationExchangeLinkRepository) {
    ...
    fun deLink(linkId: String?, adminApprovalContext: AdminApprovalContext)
    {
        ...
        organizationExchangeLinkRepository.deleteById(link.id)
    ...
    private fun linkSnapshot(link: OrganizationExchangeLink): String
    {
