...
class DomainEventDispatcherTest
{
    ...
    private fun pendingRow(eventType: String = "exchange.activated"): DomainEventOutboxEntry
    {
        val event = DomainEvent(
            type = eventType,
            subject = DomainEvent.SubjectRef("EXCHANGE", UUID.randomUUID().toString()),
        )
        return DomainEventOutboxEntry().apply {
    ...
    fun `deliverNext routes the event and marks the row delivered`()
    {
        ...
        assertEquals(DomainEventDispatcher.DeliveryOutcome.DELIVERED, outcome)
        verify(eventRouter).routeDurable(any())
        assertEquals(DomainEventOutboxEntry.STATUS_DELIVERED, row.status)
    ...
    fun `deliverNext returns NONE when no row is deliverable`()
    {
        ...
        assertEquals(DomainEventDispatcher.DeliveryOutcome.NONE, outcome)
    ...
    fun `deliverNext keeps the row pending and backs off after a routing failure`()
    {
        ...
        assertEquals(DomainEventDispatcher.DeliveryOutcome.RETRIED, outcome)
        assertEquals(DomainEventOutboxEntry.STATUS_PENDING, row.status)
    ...
    {
        val row = DomainEventOutboxEntry().apply {
        ...
        assertEquals(DomainEventDispatcher.DeliveryOutcome.FAILED, outcome)
        assertEquals(DomainEventOutboxEntry.STATUS_FAILED, row.status)
        verify(eventRouter, never()).routeDurable(any())
        verify(outboxRepository).update(row)
    }
    ...
    {
        whenever(outboxRepository.countByStatus(DomainEventOutboxEntry.STATUS_PENDING)).thenReturn(5L)
        whenever(outboxRepository.oldestPendingCreatedAt())
            .thenReturn(Timestamp.from(Instant.now().minusSeconds(120)))
        whenever(outboxRepository.countPendingWithAttemptsAtLeast(DomainEventDispatcher.REPEATED_FAILURE_THRESHOLD))
            .thenReturn(2L)
        whenever(outboxRepository.countPendingWithAttemptsAtLeast(DomainEventDispatcher.EXHAUSTED_THRESHOLD))
            .thenReturn(1L)
