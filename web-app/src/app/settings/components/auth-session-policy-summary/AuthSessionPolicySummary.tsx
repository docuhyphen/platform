...
} from "./authSessionDuration.ts";
import {useAuthSessionPolicySummaryStyles} from "./AuthSessionPolicySummaryStyles.tsx";

interface AuthSessionPolicySummaryProps
{
...
const AuthSessionPolicySummary = (
    {
        effective,
        guardrails,
    }: AuthSessionPolicySummaryProps
) =>
{
    const styles = useAuthSessionPolicySummaryStyles();
    ...
    return (
        <section id={"auth-session-policy-summary"}>
            <Text
                id={"auth-session-policy-summary-introduction"}
            ...
            <div
                id={"auth-session-policy-summary-grid"}
                className={styles.grid}>
                {items.map(item => (
                    <div
                        id={`auth-session-policy-${item.id}`}
                        className={styles.card}
                        key={item.id}>
                        <Text
                            id={`auth-session-policy-${item.id}-label`}
                            weight={"semibold"}>
                            {item.label}
                        </Text>
                        <Text
                            id={`auth-session-policy-${item.id}-value`}
                        ...
                        <Text
                            id={`auth-session-policy-${item.id}-range`}
                            className={styles.supportingText}
                            size={200}>
                            Allowed range: {item.range}
                        </Text>
                        <Text
                            id={`auth-session-policy-${item.id}-description`}
                            size={200}>
                            {item.description}
                        </Text>
                    </div>
...
export default AuthSessionPolicySummary;
