...
import {
    ...
    verifyNoAuthSharingSessionAccessCode,
...
const NoAuthSessionDocumentList: React.FC<NoAuthSessionDocumentListProps> = ({session}) =>
{
    ...
    const sessionDocuments: DocumentBasicDto[] =
        ((session as unknown as { documents?: DocumentBasicDto[] }).documents || []);
    const accessWindowDays = Math.max(1, session.noAuthAccessValidityDays ?? 7);
    ...
    const toActionableUploadError = (message: string): string =>
    {
        ...
        if (/Sharing session has already ended/i.test(message) || /session has ended/i.test(message))
        {
    ...
    const onVerifyAccessCode = async () =>
    {
        ...
        try
        {
            await verifyNoAuthSharingSessionAccessCode(session.id, trimmedCode);
    ...
    return (
        <section className={styles.container}>
            ...
            {sessionDocuments.map(renderDocumentCard)}
        </section>
    );
};
