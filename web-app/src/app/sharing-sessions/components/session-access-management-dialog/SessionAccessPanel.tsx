...
import {useSessionAccessPanelStyles} from './SessionAccessPanelStyles';
import {useAuth} from '../../../../context/AuthContext';
import {
    changeSessionAccessRole,
    grantSessionAccess,
    listSessionAccess,
    revokeSessionAccess,
} from '../../../../services/sharingSessionApi';
import {SessionAccessEntryDto, ShareConstraints} from '../../../../services/types/dtos';
import {
...
{
    return CONSTRAINED_ROLES.has(roleName as SessionShareRole);
}
...
function buildDraft(entry: SessionAccessEntryDto): AccessEntryDraft
{
    return {
...
function buildDrafts(entries: SessionAccessEntryDto[]): Record<string, AccessEntryDraft>
{
...
function entryDisplayLabel(entry: SessionAccessEntryDto): string
{
 ...
 */
function isProtectedEntry(entry: SessionAccessEntryDto, currentAppUserId?: string): boolean
{
...
function isOwnerEntry(entry: SessionAccessEntryDto): boolean
{
    return entry.roleName === 'OWNER';
}

const SessionAccessPanel: React.FC<SessionAccessPanelProps> = ({sessionId}) =>
{
    const styles = useSessionAccessPanelStyles();
    const {appUser} = useAuth();
    const [entries, setEntries] = useState<SessionAccessEntryDto[]>([]);
    ...
    const [newRole, setNewRole] = useState<string>(SessionShareRole.VIEWER);
    ...
    const loadAccess = useCallback(async () =>
    {
        ...
        try
        {
            const data = await listSessionAccess(sessionId);
            setEntries(data);
    ...
    const persistEntries = (updated: SessionAccessEntryDto[]) =>
    {
        setEntries(updated);
    ...
    const handleAddPerson = async () =>
    {
        ...
        try
        {
            ...
            const result = await grantSessionAccess(sessionId, {
                principalKind: 'USER',
            ...
            setNewPersonEmail('');
            setNewRole(SessionShareRole.VIEWER);
    ...
    const handleSaveEntry = async (entry: SessionAccessEntryDto) =>
    {
        ...
        try
        {
            const result = await changeSessionAccessRole(sessionId, entry.shareId, {
    ...
    const handleReinstate = async (entry: SessionAccessEntryDto) =>
    {
        const draft = drafts[entry.shareId] ?? buildDraft(entry);
        setBusy(true);
        setError(null);
    ...
    const handleRevoke = async (shareId: string) =>
    {
        ...
        try
        {
            const result = await revokeSessionAccess(sessionId, shareId);
            persistEntries(result);
        }
        catch (err: unknown)
        {
                    ...
                    <div className={styles.addForm}>
                        <div  className={styles.addFormRow1}>
                            ...
                            <Combobox
                                ...
                                value={AssignableRoleDisplayNames[newRole] || SessionShareRoleDisplayNames[newRole as SessionShareRole] || newRole}
                                selectedOptions={[newRole]}
                                onOptionSelect={(_e, d) => setNewRole(d.optionValue || SessionShareRole.VIEWER)}
                            >
                        ...
                        <Card key={entry.shareId} className={styles.row}>
                            ...
                            {showDetails && (
                                <div className={styles.details}>
                                    ...
                                    <div className={styles.detailsRow1}>
                                        {isImmutable ? (
                                            <Text weight="semibold">
                                                {SessionShareRoleDisplayNames[draft.roleName as SessionShareRole] || draft.roleName}
                                            </Text>
                                        ) : (
                                            <Combobox
                                                value={SessionShareRoleDisplayNames[draft.roleName as SessionShareRole] || draft.roleName}
                                    ...
                                    {!isProtected && isOwner && (
                                        <Text size={200}>Session owner access is required and cannot be changed or
                                            revoked.</Text>
                                    )}
...
export default SessionAccessPanel;
